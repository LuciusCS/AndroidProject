...
//    private static final String Ciper_Mode = "AES/ECB/PKCS5PADDING";
    ...
    //用于表示秘钥种子
    private static final String Key_Seed="EASTSOFTPADESBLE";

    //用于根据生成的秘钥对明文plainText进行加密
    public static final String encrypt(String plainText) {
//        Key secretKey = getKey(Key_Seed);
//        String secretKey = "1234567887654321";// AES加密要求key必须要128个比特位（这里需要长度为16，否则会报错）
          byte[] raw   = Key_Seed.getBytes();   //获得原始对称秘钥字节数组
...
//            LogUtils.printInfo(new String(result,"UTF-8"));
