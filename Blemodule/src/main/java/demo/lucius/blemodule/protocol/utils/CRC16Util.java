...
public class CRC16Util {
     ...
     * @return 
     */
    public static byte[] CRC_16_X25(byte[] message) {
        int crc_reg = 0xffff;//CRC校验时初值
        for (int i = 0; i < message.length; i++) {
            crc_reg = (crc_reg >> 8) ^ crc16_ccitt_table[(crc_reg ^ message[i]) & 0xff];
        }
        //返回两个自己倒叙
        return  ProtocolUtil.hexStringToByte(ProtocolUtil.reverseCode(ProtocolUtil.appendCode(String.valueOf(~crc_reg),4)));
    }
